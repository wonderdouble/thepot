...
function makeBlogQuery({
  ...
  async function getBlog({
    ...
    return (await db.collection('Blog').find(query).sort({
      date: -1
    }).limit(Number(max)).toArray()).map(documentToBlog);
  }
  ...
  async function add({
    ...
    const {
      result,
      ops
    } = await db.collection('Blog').insertOne(blog).catch(mongoError => {
  ...
  async function findById({
    ...
    const db = await database;
    const found = await db.collection('Blog').findOne({
      _id: db.makeId(id)
    });
  ...
  async function findByMin({
    ...
    const db = await database;
    return (await db.collection('Blog').find().sort({
      date: -1
    }).limit(Number(min)).toArray()).map(documentToBlog);
  }
  ...
  async function findByTopic({
    ...
    const db = await database;
    return (await db.collection('Blog').find({
      topic: topic
    }).sort({
      date: -1
  ...
  async function update({
    ...
    const {
      result
    } = await db.collection('Blog').updateOne(query, newSet, {
      upsert: true
    });
  ...
  function documentToBlog({
    _id: id,
    ...doc
  }) {
    return (0, _blog.default)({
      id,
      ...doc
    });
  }
